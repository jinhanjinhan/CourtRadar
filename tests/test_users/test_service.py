...
async def test_create_user_success():
    ...
    with (
        patch(
            "app.modules.users.service.repository.get_user_by_email",
            new_callable=AsyncMock,
        ) as mock_get,
        patch(
            "app.modules.users.service.repository.create_user", new_callable=AsyncMock
        ) as mock_create,
    ):
        mock_get.return_value = None
        mock_create.return_value = user
        result = await create_user(
            session, UserCreate(email="test@example.com", display_name="Test User")
        )
    assert result == user
...
async def test_create_user_email_already_exists_raises():
    ...
    existing = make_user()
    with patch(
        "app.modules.users.service.repository.get_user_by_email", new_callable=AsyncMock
    ) as mock_get:
...
async def test_get_user_found():
    ...
    session = AsyncMock()
    with patch(
        "app.modules.users.service.repository.get_user_by_id", new_callable=AsyncMock
    ) as mock_get:
