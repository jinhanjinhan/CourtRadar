...
def test_create_alert_preference_success(client):
    ...
    with patch("app.modules.alerts.router.create_alert_preference", new_callable=AsyncMock) as mock:
        mock.return_value = alert
        response = client.post("/alert-preferences", json={"user_id": 1, "venue": "Arena", "max_price": 25.0})
...
def test_list_alert_preferences_returns_all(client):
    ...
    with patch("app.modules.alerts.router.list_alert_preferences", new_callable=AsyncMock) as mock:
