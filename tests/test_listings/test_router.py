...
def test_create_parsed_listing_success(client):
    ...
    with patch("app.modules.listings.router.create_parsed_listing", new_callable=AsyncMock) as mock:
...
def test_list_parsed_listings_returns_all(client):
    ...
    with patch("app.modules.listings.router.list_parsed_listings", new_callable=AsyncMock) as mock:
...
def test_match_listing_found_no_matches(client, mock_db):
    listing = make_listing(id=1, venue="Arena", price=10.0, start_time=time(20, 0), end_time=time(22, 0))
    mock_result = MagicMock()
    mock_result.scalars.return_value.all.return_value = []
    mock_db.execute = AsyncMock(return_value=mock_result)
    with patch("app.modules.listings.router.get_parsed_listing", new_callable=AsyncMock) as mock_get:
...
def test_match_listing_found_with_matches(client, mock_db):
    listing = make_listing(id=2, venue="Arena", price=10.0, start_time=time(20, 0), end_time=time(22, 0))
    # Alert that matches the listing
    alert = make_alert(id=5, venue="Arena", max_price=15.0, earliest_time=time(19, 0), latest_time=time(23, 0))
    mock_result = MagicMock()
...
def test_match_listing_alert_price_too_high_excluded(client, mock_db):
    listing = make_listing(id=3, venue="Arena", price=30.0, start_time=time(20, 0), end_time=time(22, 0))
