...
@router.post(
    "/users", response_model=CreatedResponse, status_code=status.HTTP_201_CREATED
)
async def create_user_route(
    payload: UserCreate, session: AsyncSession = Depends(db_session)
) -> CreatedResponse:
    try:
...
async def get_user_route(
    user_id: int, session: AsyncSession = Depends(db_session)
) -> UserRead:
