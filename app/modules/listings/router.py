...
async def list_parsed_listings_route(
    session: AsyncSession = Depends(db_session),
) -> list[ParsedListingRead]:
...
async def match_listing_route(
    listing_id: int, session: AsyncSession = Depends(db_session)
) -> dict[str, object]:
    ...
    result = await session.execute(
        select(AlertPreference).where(AlertPreference.is_active.is_(True))
    )
    matches = [
        alert
